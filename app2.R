...
server <- function(input, output,session) {
  ...
  data.valyear.Gender <- data.mortality %>%
    group_by(Val.Year,Gender) %>%
    summarise_at(vars(Deaths,qx2015VBT.policy),list(sum)) %>%
    mutate(atoe = Deaths/qx2015VBT.policy)
  
  output$atoe_plot <- renderPlotly({
    ggplot(data.mortality.valyear,aes(x = Val.Year, y = atoe)) + geom_point() + geom_line()
  })
  
  output$death_plot <- renderPlotly({
    ggplot(data.mortality.valyear,aes(x = Val.Year, y = Deaths)) + geom_bar(stat="identity")
  })
  
  output$bivariate_atoe <- renderPlotly({
    ggplot(data.valyear.Gender,aes(x = Val.Year,y = Gender)) + 
